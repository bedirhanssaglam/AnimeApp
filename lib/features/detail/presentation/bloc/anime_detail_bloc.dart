...
class AnimeDetailBloc extends Bloc<AnimeDetailEvent, AnimeDetailState> {
  ...
  AnimeDetailBloc(this.animeDetailRepository) : super(const AnimeDetailState()) {
    on<FetchAnimeCharactersEvent>(_onFetchAnimeCharactersEvent);
  }

  Future<void> _onFetchAnimeCharactersEvent(FetchAnimeCharactersEvent event, Emitter<AnimeDetailState> emit) async {
    try {
      final AnimeCharacters animeCharacters = await animeDetailRepository.fetchAnimeCharacters(event.id);

      emit(
        state.copyWith(status: AnimeDetailStatus.success, animeCharacters: animeCharacters),
      );
    } catch (_) {
